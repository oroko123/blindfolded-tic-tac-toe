...
public:
  ...
  long long countMatrixSize(const GameState2 &state) {
    ...
    long long count = 1;
    if (res != ONGOING) {
      int win_coef = res == PLAYER1WIN ? 1 : (res == PLAYER2WIN ? -1 : 0);
      if (win_coef == 0) {
        return count;
      }
      ...
      for (auto &[key, value] : izometries) {
        ...
        // state.printBoard();
        // cerr << playerToSign(player_turn) << " " << current_value << " " <<
        // key
        //      << endl;
      ...
      player_matrix[player_turn][{player_state_key, player_state_key / 2}] = -1;

      for (auto &[state, move, key] : new_states_with_moves_and_keys) {
        (void)key;
        (void)move;
        count += countMatrixSize(state);
      }
...
private:
  ...
  map<array<PlayerKey, 2>, int> player_matrix[2]; // <state_key, action_key
...
void test_do_not_lose_move() {
  ...
  Counter counter;
  counter.countMatrixSize(initialGameState);
  counter.printSolution();
  Serializer::write_to_file(GetMatrixFilename("P", /*lose_move=*/false),
                            counter.getP(), false);
  Serializer::write_to_file(GetMatrixFilename("C", /*lose_move=*/false),
                            counter.getC(), false);
  Serializer::write_to_file(GetMatrixFilename("D", /*lose_move=*/false),
                            counter.getD(), false);
}
...
void test_lose_move() {
  ...
  counter.countMatrixSize(initialGameState);
  counter.printSolution();
  Serializer::write_to_file(GetMatrixFilename("P", /*lose_move=*/true),
                            counter.getP(), false);
  Serializer::write_to_file(GetMatrixFilename("C", /*lose_move=*/true),
                            counter.getC(), false);
  Serializer::write_to_file(GetMatrixFilename("D", /*lose_move=*/true),
                            counter.getD(), false);
}
